...
def runSimulation(worDir, cmd, env):
    """ Run the simulation.
...
#                            raise ValueError(msg)
    ...
    def _getDymolaTranslationStatistics(self, data, warnings, errors):
        ...
        fulFilNam = os.path.join(data['ResultDirectory'], data['dymola']['TranslationLogFile'])
...
to access a summary of the comparison results.\n""".format(
    ...
    def _write_runscript_dymola(self, iPro, tra_data_pro):
        ...
        runFil = open(os.path.join(self._temDir[iPro], "runAll.mos"), mode="w", encoding="utf-8")
        runFil.write(
            f"""
...
""")
        ...
        runFil.write(f"""
openModel("{self.getLibraryName()}/package.mo", changeDirectory=false)
...
""" % self._statistics_log)
        ...
        for i in range(nTes):
            ...
            if self._isPresentAndTrue(
                ...
                absMosFilNam = os.path.join(self._temDir[0], mosFilNam)
                values = {
                    "libraryInstallDirectory": f"../{os.path.split(self._temDir[0])[-1]}",
...
"""
            ...
            if self._isPresentAndTrue('translate', tra_data_pro[i]['dymola']):
                ...
                template = r"""
{set_non_pedantic}
rScript=RunScript("{libraryInstallDirectory}/{libraryName}/Resources/Scripts/Dymola/{scriptFile}");
{set_pedantic}
...
RunScript("{libraryInstallDirectory}/{libraryName}/Resources/Scripts/Dymola/{scriptFile}");
savelog("{model_name}.translation.log");
if Modelica.Utilities.Files.exist("dslog.txt") then
  Modelica.Utilities.Files.move("dslog.txt", "{model_name}.dslog.log");
end if;
...
Modelica.Utilities.Streams.print("        \"command\" :\"RunScript(\\\"{libraryInstallDirectory}/{libraryName}/Resources/Scripts/Dymola/{scriptFile}\\\");\",", "{statisticsLog}");
Modelica.Utilities.Streams.print("        \"translationLog\"  : \"{translationLog}\",", "{statisticsLog}");
Modelica.Utilities.Streams.print("        \"result\"  : " + String(iSuc > 0), "{statisticsLog}");
"""
                runFil.write(template.format(**values))
...
""")
    ...
    def _setTemporaryDirectories(self):
        ...
        # copy library into the directory and
        # write run scripts to directory
        main_dir = self._temDir[0]
        for iPro in range(self._nPro):
            new_bui_dir = os.path.join(self._temDir[iPro], self.getLibraryName())
            if iPro == 0:
                shutil.copytree(
                    self._libHome,
                    new_bui_dir,
                    ...
                    ignore=shutil.ignore_patterns(
                        ...
                        '__pychache__'))
#            else:
#                # For other processors, just make a symbolic link
#                src = os.path.join(main_dir, os.path.split(self._libHome)[-1])
#                os.symlink(src = src, dst = new_bui_dir, target_is_directory = True)
        # Make sure everything is written to disc before proceeding.
        os.sync()
        return
...
#                tem_dir.append(di)
        ...
        if not self._useExistingResults:
            ...
            if 'MODELICAPATH' in os.environ:
                env['MODELICAPATH'] = "{}:{}".format(self._temDir[0], os.environ['MODELICAPATH'])
            else:
                env['MODELICAPATH'] = self._temDir[0]

            if self._modelica_tool == 'openmodelica':
                env['MODELICAPATH'] = f"{env['MODELICAPATH']}"
                env['OPENMODELICALIBRARY'] = env['MODELICAPATH']
            ...
            if self._nPro > 1:
                ...
                po.map(functools.partial(runSimulation,
                                         cmd=cmd,
                                         env=env),
                       [x for x in self._temDir])
                po.close()
                po.join()
            else:
                if len(self._data) > 0:
                    runSimulation(self._temDir[0], cmd, env)
            ...
            if self._modelica_tool == 'dymola':
                with open(self._statistics_log, mode="w", encoding="utf-8") as logFil:
                    stat = list()
                    for d in self._temDir:
                        temLogFilNam = os.path.join(d, self._statistics_log)
                        if os.path.exists(temLogFilNam):
