...
def runSimulation(worDir, libDir, cmd):
    """ Run the simulation.

    :param worDir: The working directory.
    :param libDir: The directory that contains the library to be tested.
...
class Tester(object):
    ...
    *Regression testing using OPTIMICA or JModelica*
    ...
    def __init__(
        ...
        # Temporary directory that will contain a copy of the library
        self._temLibDir = None
...
'%s' does not exist.""" % (pac, rooPat)
    ...
    def _getTranslationStatistics(self, data, warnings, errors):
        ...
        fulFilNam = os.path.join(data['ResultDirectory'], data['TranslationLogFile'])
    ...
    def _check_fmu_statistics(self, ans):
        ...
        for data in self._data:
            ...
            if self._includeFile(data['ScriptFile']) and data['mustExportFMU']:
                ...
                fmu_fil = os.path.join(data['ResultDirectory'], data['FMUName'])
                try:
...
#                    break
        ...
        for iPro in range(self._nPro):
            ...
            if self._modelica_tool in ['dymola', 'omc']:
                ...
                if self._modelica_tool == 'dymola':
                    ...
                    runFil.write(
                        ('cd(\"{}\");\n'.format(self._temDir[iPro])).replace("\\", "/"))
                    runFil.write('openModel("{}/{}/package.mo", changeDirectory=false, mustRead=false);\n'.format(
                         self._temLibDir.replace("\\", "/"), self.getLibraryName()))
                elif self._modelica_tool == 'omc':
                    runFil.write('loadModel(Modelica, {"3.2"});\n')
                    runFil.write('getErrorString();\n')
                    runFil.write(f'loadModel({self._getLibraryName()});\n')
    ...
    def _setTemporaryDirectories(self):
        ...
        # Make temporary directory that will contain the library
        self._temLibDir = dirNam = tempfile.mkdtemp(
                prefix='tmp-' + self.getLibraryName() + '-lib-')

        # Copy library. This is done so edits on the library can happen while the
        # unit tests run in an isolated temporary directory.
        shutil.copytree(self._libHome,
                        os.path.join(self._temLibDir, self.getLibraryName()),
                        symlinks=True,
                        ignore=shutil.ignore_patterns(
                            '.svn', '.mat', 'request.', 'status.',
                            'tmp-*'
                            'funnel_comp',
                            'ReferenceResults',
                            'compareVars'))

        # Get temporary directory for simulations
        self._temDir = []
        for iPro in range(self._nPro):
    ...
    def run(self):
        ...
        if not self._useExistingResults:
            ...
            if self._nPro > 1:
                ...
                if len(self._data) > 0:
                    runSimulation(worDir=tem_dir[0], libDir=self._temLibDir, cmd=cmd)
