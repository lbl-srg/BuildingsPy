...
# return
    ...
    def _getSimulationResults(self, data, warnings, errors):
        ...
        try:
            ...
            # HACK #2
            try:
                list_files = [l for l in os.listdir(data['ResultDirectory']) if re.search('result.mat$', l)]
                infer_name = difflib.get_close_matches(data['ResultFile'], list_files, n=1)[0]
                fulFilNam = re.sub(data['ResultFile'], infer_name, fulFilNam)
                # HACK #1
                r = Reader(fulFilNam, "dymola")
            ...
            errors.append("Failed to load %s generated by %s.\n%s\n" %
                (fulFilNam, data['ScriptFile'], e))
            return ret
