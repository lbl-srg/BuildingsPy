    ...
    def merge(self):
        ...
        if os.path.isfile(copFilPat):
            with open(copFilPat,'r') as fp:
                files = fp.readlines()
                for fil in files:
                    if not fil.startswith("#") and os.path.isfile(fil):
                        os.remove(fil)
        
        copiedFiles=list()
        copiedFiles.append("# Do not edit this file unless you know what you are doing.")
        copiedFiles.append("# This file is used by the Annex60 merge script and generated by BuildingsPy.")
